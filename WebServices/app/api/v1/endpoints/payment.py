...
def filter_loan_providers(data):
    ...
    count = 1
    for entry in data:
        logger.debug(f"Count: {count}  {entry}")
        filtered_entry = {
            "option": count,
            "details": {
                "loan_package_name": entry.get("loan_package_name"),
                ...
                "min_cibil_score": entry.get("min_cibil_score"),
...
async def get_loan_package_by_cibil_score(cibil_score: int, lender_name: Optional[str] = "", mongoDalService: MongoDBServiceLoan = Depends(MongoDalServiceLoan)):
    """
    ...
    try:
        ...
        if lender_name:
            loan_package = mongoDalService.mognoDal.get_loan_package_by_cibil_score_lender_name(cibil_score, lender_name)
        else:
...
# get api to list available lenders
@router.get("/all_lenders/", description="List all available lenders")
async def list_lenders(limit: Optional[int] = None, mongoDalService: MongoDBServiceLoan = Depends(MongoDalServiceLoan)):
    """
    ...
    """
    try:
        logger.info(f"=====list_lenders::::Fetching all lenders=====")
        lenders = mongoDalService.mognoDal.get_available_lenders(limit)
        if not lenders:
            return JSONResponse(
                status_code=status.HTTP_404_NOT_FOUND,
                content = {
        ...
        logger.debug(f"lenders: {lenders}") 
        lender_list = [provider_name for provider_name in lenders]
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content = {
                "status_code": status.HTTP_200_OK,
        ...
        logger.error(f"list_lenders:: Error fetching lenders: {e}")
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content = ({
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# write delete api to delete all loan packages
@router.delete("/", description="Delete all loan packages")
async def delete_all_loan_packages(mongoDalService: MongoDBServiceLoan = Depends(MongoDalServiceLoan)):
    """
    ...
    """
    try:
        logger.info(f"=====delete_all_loan_packages::::Deleting all loan packages=====")
        if mongoDalService.mognoDal.delete_all_loan_packages():
            return JSONResponse(
        ...
        logger.error(f"delete_all_loan_packages:: Error deleting loan packages: {e}")
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content = ({
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
                "message": "Error deleting loan packages"
            })
        )

...
async def generate_otp_for_phone_number(phone_number: str, mongoDalService: MongoDalServiceOTP):
    ...
    try:
        ...
        return JSONResponse(
            ...
            content=OTPResp(
                status_code=status.HTTP_200_OK,
                message=f"OTP {otp} sent successfully to this phone number: {phone_number} for verification",
...
async def validate_otp(otp: int, mongoDalService: MongoDalServiceOTP, mongoDalServiceDEAL: MongoDalServiceDeal):
    """
    Validate OTP
    Args:
    ...
    try:
        ...
        if otp_obj["otp"] == otp:
            ...
            deal_data = mongoDalServiceDEAL.mognoDal.fetch_customer_details_by_phone_number(otp_obj["phone_number"])
            logger.debug(f"Deal data fetched: {deal_data}")
            return JSONResponse(
                status_code=status.HTTP_200_OK,
                content=CommonResp(
                    status_code=status.HTTP_200_OK,
                    message="OTP verified successfully",
                    data=serialize_doc(deal_data)
                ).dict(),
            )

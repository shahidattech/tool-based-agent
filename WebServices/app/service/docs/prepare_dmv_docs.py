...
async def generate_contract_pdf(data, file_path='customer_contract.pdf'):
    ...
    try:
        ...
        elements.append(Paragraph(f"Contact: {data['phone_primary']}", normal_style))
        elements.append(Paragraph(f"Contact: {data['phone_secondary']}", normal_style))
        elements.append(Paragraph(f"Contact: {data['address']}", normal_style))

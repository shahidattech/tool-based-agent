...
async def create_deal(payload: Deal, mongoDalService: MongoDalService):
    """
    Create a deal
...
async def get_deal(deal_number: str, check_empty: bool, mongoDalService: MongoDalService):
    """
    ...
    """
    try:
        logger.info(f"=====Getting deal====={deal_number}")
        deal = mongoDalService.mognoDal.collection.find_one({"deal_number": deal_number})
        if deal:
            # Convert ObjectId to string
            if "_id" in deal:
                deal["_id"] = str(deal["_id"])
            
            filtered_desired_dict = filter_desired_values(deal)
            
            if check_empty:
                empty_fields_deal = filter_empty_values(filtered_desired_dict["vehicle"])

                if empty_fields_deal:
                    return JSONResponse(
                        status_code=status.HTTP_200_OK,
                        content=CreateDealResp(
                            status_code=status.HTTP_200_OK,
                            message="Deal fetched successfully with empty fields in vehicle",
                            data={"vehicle": empty_fields_deal}
                        ).dict(),
                    )

            return JSONResponse(
                status_code=status.HTTP_200_OK,
                content=filtered_desired_dict
            )
        else:
            return JSONResponse(
...
async def update_deal(deal_number: str, payload:DealToUpdate, mongoDalService: MongoDalService):
    """
    ...
    """
    try:
        logger.info(f"=====Updating deal====={deal_number}")
        # Convert payload to dict and exclude unset values
        payload_dict_to_update = payload.dict(exclude_unset=True)
        existing_deal = mongoDalService.mognoDal.collection.find_one({"deal_number": deal_number})
        if existing_deal:
            ...
            mongoDalService.mognoDal.collection.update_one({"deal_number": deal_number}, {"$set": updated_deal})
            return JSONResponse(
                status_code=status.HTTP_200_OK,
                content=CreateDealResp(
                    status_code=status.HTTP_200_OK,
